...
<head>
    ...
    <script>
        ...
        //新建
        function Add(){
            postURL = "/jsons/addOrganization.form";
            $("#Form").form("clear");
        ...
        //修改
        function Edit(){
            postURL = "/jsons/editOrganization.form";
            var row = $('#tt').datagrid('getSelected');
            if (row){
                childrenId = [];
                getChildrenId(row);//获取当前菜单的所有子菜单的ID
                if(row.organizationId=="2c659331-0d1a-11e6-b867-0025b6dd0800"){
                    ShowMsg("菜单根目录不允许修改");
                    return;
                }
                $("#Form").form("clear");
        ...
        function Delete(){
            postURL = "/jsons/deleteOrganization.form";
            var row = $('#tt').datagrid('getSelected');
            if (row){
                childrenId = [];
                getChildrenId(row);//获取当前菜单的所有子菜单的ID
                ...
                $.messager.confirm('提示', message, function(result){
                    if (result){
                        $.post("/organization/deleteMenu.form",{
                            ...
                            if(data=="1"){
                                ...
                                //重新加载数据
                                $('#tt').treegrid('reload');
                                $('#organizationParentId').combotree({url: '/organization/getParentMenu.form',required: true});
                            }else {
        ...
        function Save(){
            if($("#Form").form('validate')){
                ...
                if(isContainStr(childrenId,jsonObject.organizationParentId)){
        ...
        function getChildrenId(row){
            childrenId.push(row.organizationId);
...
<table id="tt" title="" class="easyui-treegrid" style="width:100%;height: 100%;"
       data-options="
                    url: '/organization/getSysMenuList.form',
                    method: 'post',
                    rownumbers: true,
    ...
    <thead>
    <tr>
        <th field="organizationName" width="15%">组织名称</th>
        <th field="organizationId" hidden>组织ID</th>
        <th field="organizationSort" width="10%">组织排序</th>
        <th field="organizationParentId" hidden>父级组织ID</th>
    </tr>
    </thead>
</table>
<%--对话框--%>
<div id="dlg" class="easyui-dialog hide" title=""
    ...
    <form id="Form">
        <div style="margin-bottom:10px;margin-top: 10px;display: none;">
            ...
            <input class="easyui-textbox" name="organizationId" id="organizationId">
        </div>
        <div style="margin-bottom:10px;margin-top: 10px;">
            <div class="word">组织名称:</div>
            <input class="easyui-textbox" data-options="required:true,validType:'length[1,25]'"
                   name="organizationName" id="organizationName" style="width:100%;height:32px">
        </div>
        <div style="margin-bottom:10px;margin-top: 10px;">
            <div class="word">父级组织:</div>
            <input class="easyui-combotree" style="height:32px;width:100%;"
                   name="organizationParentId" id="organizationParentId"
                   data-options="required:true,url:'/organization/getParentMenu.form',method:'post'">
        </div>
        <div style="margin-bottom:10px;margin-top: 10px;">
            <div class="word">组织排序:</div>
            <input class="easyui-numberspinner" value="1" data-options="increment:1" min="0" maxlength="8"
                   style="height:32px;width:100%;" name="organizationSort" id="organizationSort">
        </div>
    </form>
</div>
<%--对话框保存、取消按钮--%>
