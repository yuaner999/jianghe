...
<head>
    ...
    <script type="text/javascript">
        ...
        var moduleType="bc50fa33-fff6-11e6-9450-38d547ab495f";
        var introdata;
        var newsClass = null;
        var type = null;
        var sqlStr = null;
        var newsDescribe = null;
        var totalNUm=100;
        var page=1;
        var rows=100;
        $(function(){
        ...
        function genner(data){
            ...
            OrgShows.BoxTemplet="<div onclick='checkall(this)' id=\"{Id}\" class=\"OrgBox\"><span>{Name}</span><a href='www.baidu.com'></div>";
            OrgShows.Run();
            reSetRootNodeCss();
        }
        function  checkall(object){
            var name=$(object).find('span').text();
            for(var j=0;j<introdata.length;j++)
            {
                if(introdata[j].newsTitle==name)
                {
                    window.location.href="detail.form?newsId="+introdata[j].newsId;
                }
            }
           /* $.ajax({
                url:"/jsons/fontloadidorganization.form",
                type:"post",
                data:{organizationId:id},
                dataType:"json",
                success:function(data){
                    for (var i;i<data.rows.length;i++){
                        if (data.rows[i].organizationParentId !="2c659331-0d1a-11e6-b867-0025b6dd0800" &&data.rows[i].organizationParentId !=null){
                            window.location.href="detail.form?name="+name;
                        }
                    }
                },
                error: function () {
