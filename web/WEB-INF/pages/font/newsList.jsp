...
<head>
    ...
    <script type="text/javascript">
        ...
        {
            $.ajax({
                url: "/jsons/loadMenu.form",
                type: "post",
                data:{sysmenu_id:moduleType},
                datatype: "json",
                success: function (data) {
                    if(data.total>0)
                    {
                        var titleName = data.rows[0].sysmenuname;
                        if (data.rows[0].sysmenuname != "教育管理" && data.rows[0].sysmenuname != "管理下载") {
                            titleName = titleName.replace("管理", "");
                        }

                        $("#titleName").html(titleName);
                        if (titleName == "学院新闻") {
                            $("#isnews").html("News");
                            $("#newsimg").attr("src", "<%=request.getContextPath()%>/asset_font/images/imgsound_11.png");
                        }
                    }
                }
            })
                    ...
                    if(data.rows&&data.rows.length>0){
                       ...
                       /* var titleName=data.rows[0].sysmenu_name.replace("管理","");
                        ...
                        for(var i=0;i<data.rows.length;i++){
                            var row=data.rows[i];
                            var date="";
                            if(data.rows[i].newsDate!=null)
                            {
                                date=data.rows[i].newsDate.substring(0,data.rows[i].newsDate.indexOf(" "));
                            }
                            str='<li>'+
                                    '<a href="javascript:void(0)" onclick="Download(\''+row.newsFile+'\');">'+
                                    '<div></div>'+
                                    '<b>'+row.newsTitle+'</b>'+
                                    '<span style="margin-left: 10px;margin-right: 20px">'+date +'</span>'+
        ...
        function reload_4(){
            $.ajax({
                ...
                success:function(data){
                    ...
                    if(data.rows&&data.rows.length>0){
                        totalNUm=data.total;
                        var titleName=data.rows[0].sysmenu_name;
                        var title1="实验中心实验员";
                        var title2="城乡规划系讲师、助教";
                        var title3="城乡规划系副教授";
                        var title4="城乡规划系教授";
                        var title5="建筑系讲师、助教";
                        var title6="建筑系教授";
                        var title7="建筑系副教授";
                        if(titleName==title1||titleName==title2||titleName==title3||titleName==title4||titleName==title5||titleName==title6||titleName==title7)
                        {
                            for(var i=0;i<data.rows.length;i++){
                                str='<li>'+
                                        ...
                                        '<b>'+data.rows[i].newsTitle+'</b>'+
                                        '<span>'+'</span>'+
                                        '<span style="margin-left: 10px">'+data.rows[i].newsDescribe+'</span>'+
                                        '</a>'+
                                        '</li>';
                                str=str.replace(/null/,"");
                                $("#newsContent").append(str);
...
<div class="section">
    ...
    <div class="news_box">
        <div class="news">
            <p>
                <span id="titleName" style="font-size: 18px"></span><span id="isnews"></span>
                <img id="newsimg" src="" />
            </p>
            <!--新闻列表
                    -->
