...
function Commend()
{
    ...
    var row = $('#dg').datagrid('getSelected');
    if (row){
        $.messager.confirm('提示', '确认推荐到学院新闻吗?', function(result){
            if (result){
                //删除数据库记录
                var type="f18f1f50-c039-11e6-9c1a-00ac7dc2f176";
                var selectId = row[deleteId];
                $.ajax({
                ...
                /*var deleteJsonObject = eval("("+"{'"+deleteId+"':'"+selectId+"'"+moduleType+"':'"+type+"'"+newsTitle+"':'"+newsdata.newsTitle+"'"
                    +newsClass+"':'"+newsdata.newsClass+"'"+newsDate+"':'"+newsdata.newsDate+"'"+newsIcon +"':'"+newsdata.newsIcon +"'"+newsAuthor+"':'"+newsAuthor+"'"
                    +newsDescribe+"':'"+newsdata.newsDescribe+"'"+isDelete+"':'"+newsdata.isDelete+"'"+isTop+"':'"+newsdata.isTop+"'"+newsContent+"':'"+newsdata.newsContent+"'"
                    +newsFile+"':'"+newsdata.newsFile+"'}"+")");*/
                var deleteJsonObject = {};
                deleteJsonObject["deleteId"]=selectId;
                deleteJsonObject["moduleType"]=type;
                deleteJsonObject["newsTitle"]=newsdata.newsTitle;
                deleteJsonObject["newsClass"]=newsdata.newsClass;
                deleteJsonObject["newsDate"]=newsdata.newsDates;
                deleteJsonObject["newsIcon"]=newsdata.newsIcon;
                deleteJsonObject["newsAuthor"]=newsdata.newsAuthor;
                deleteJsonObject["newsDescribe"]=newsdata.newsDescribe;
                deleteJsonObject["isDelete"]=newsdata.isDelete;
                deleteJsonObject["isTop"]=newsdata.isTop;
                deleteJsonObject["newsContent"]=newsdata.newsContent;
                deleteJsonObject["newsFile"]=newsdata.newsFile;
                deleteJsonObject["createMan"]=newsdata.createMan;
                deleteJsonObject["createDate"]=getNowFormatDate();
...
function Edit(){
    ...
    if (row){
        $("#Form").form("clear");
        $('#Form').form('load', row);
        if(editorName &&  editorName!="null"){//为KindEditor赋值
            KindEditor.instances[0].html(row[editorName]);
        }
        $("#user_photo").attr("src","/Files/Images/"+$("#photo_textbox").val());
        $("#dlg").dialog({title: "修改"});
        $("#dlg").dialog('open');
        $("#dlg").get(0).scrollTop=0;
    }else {
...
function Save(){
    if($("#Form").form('validate')){
        ...
        if(!jsonObject["newsClass"]){
            jsonObject["newsClass"]='';
        }
        if(!jsonObject["studentBirthday"]){
            jsonObject["studentBirthday"]=null;
        }
