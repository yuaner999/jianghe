function $nodeId(id){return document.getElementById(id)}
//计算最外面的box的尺寸
var top_Max=0;
var left_Max=0;
var nodeid;
...
function OrgNode(){
	...
	this.inIt= function(parentBoxId){
		if(this.inIted==true)return;
		var tempDiv=document.createElement("DIV");
		if(parentBoxId)
			document.getElementById(parentBoxId).appendChild(tempDiv);
		else
...
function OrgShow(OrgNode_){
			...
			for(var m=0; m<tempNodes.length; m++){
				if(n!=this.Depth){//设置底线条
					if(tempNodes[m].Nodes.length!=0){
						...
						var tempBottomLine=new CreateLine(2, tempLineTop, tempLineLeft, tempLineHeight, this.LineSize, this.LineColor, "LineBottom_"+ tempNodes[m].Id,this.parentBoxId);
				...
				if(n!=1){//设置顶线条
					...
					if(this.DepthGroup[tempNodes[m].Depth].NodeGroups[tempNodes[m].NodeGroupId].length==1){//如果只有一个节点
						...
						var temptopLine=new CreateLine(2, tempLineTop, tempLineLeft, tempLineHeight, this.LineSize, this.LineColor, "LineTop_"+ tempNodes[m].Id,this.parentBoxId);
		...
		for(var n=2; n<=this.Depth; n++){//设置横线条
			...
			for(var m=0; m<tempNodeGroups_.length; m++){
				if(tempNodeGroups_[m].length!=1){
					...
					var tempGroupLine=new CreateLine(1, tempLineTop, tempLineLeft, tempLineWidth, this.LineSize, this.LineColor, "LineGroup_"+ tempNodeGroups_[m][0].Id,this.parentBoxId);
				}
			}
		}
	...
	//*************************************************************************************************
		...
		if(This.parentBoxId)
			console.log("left_Max:"+left_Max+"|"+"width_max:"+width_max);
			$("#"+This.parentBoxId).css({"height":top_Max+height_max+"px"});//"width":left_Max+width_max+"px",
	}
	...
	function CreateLine(type_, top_, left_, long_, size_, color_, id_,parentBoxId){
		this.Type=type_;
		top_=top_+"";
		left_=left_+"";
		long_=long_+"";
		...
		if(parentBoxId)
			document.getElementById(parentBoxId).appendChild(this.Line);
		else
