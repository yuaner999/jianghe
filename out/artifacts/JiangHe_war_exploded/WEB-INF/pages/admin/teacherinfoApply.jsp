...
<head>
    ...
    <script>
        ...
        function Add(){
            postURL = addUrl;
            clickStatus = "add";
            if(editorName!="null"){//给KindEditor 初始赋值
                /*$.ajax({
                    url:"/jsons/loadModel.form",
                    type:"post",
                    dataType:"json",
                    success:function(data){
                        if(data.rows&&data.rows.length>0){
                            for(var i=0;i<data.rows.length;i++){
                                var row=data.rows[i];
                                KindEditor.instances[0].html(row.newsContent);
                            }
                        }
                    }
                })*/
        ...
        function Saves(){
            if($("#Form").form('validate')){
                ...
                jsonObject["moduleType"] = "7e5ecace-c031-11e6-9c1a-00ac7dc2f176";
...
<div id="dlg" class="easyui-dialog hide" title=""
    ...
    <div style="margin-bottom:10px;margin-top: 10px;">
        ...
        <textarea id="newsContent" name="newsContent" style="width:98%;height:300px;"></textarea>
    </div>
</div>
<%--对话框保存、取消按钮--%>
<div id="dlg-buttons">
