        ...
        $(function(){
            ...
            if(type==1){
                var str='<li class="first">'+
                        ...
                        '<li>'+
                        '<div>'+
                        '<a href="newsList.form?newsDescribe=已通过&newsClass=专职教师&type=3">专职教师</a>'+
                        '</div>'+
                        '</li>'+
                        '<li>'+
                        '<div>'+
                        '<a href="newsList.form?newsDescribe=已通过&newsClass=兼职教师&type=3">兼职教师</a>'+
                        '</div>'+
                        '</li>';
                $(".navga").append(str);
                $("#link_father").html('<a href="">师资队伍</a>');
                $("#link_son").html('<a href="">'+newsClass+'</a>');
                //具体数据
                $.ajax({
                    ...
                    success:function(data){
                        if(data.rows != null&&data.rows.length>0){
                            $(".contTxt").html('');
                            var row=data.rows[0];
                            var str= '<p>'+newsClass+'</p><div class="writer">'+
                                     '<span>发布人：</span><span>'+(row.createMan?row.createMan:'')+'</span> <span>'+(row.createDates?row.createDates:'')+'</span></div>'+
                ...
                $.ajax({
                    ...
                    success:function(data){
                        if(data.rows != null&&data.rows.length>0){
                            ...
                            var sonName=data.rows[0].sysmenu_name.replace("管理",'');
                            ...
                            for(var i = 0; i < data.rows.length; i++){
                                ...
                                str2=   '<p id="sonName">'+(sonName?sonName:'')+'</p>'+
                                        '<div class="writer">'+
                                        '<span>发布人：</span>'+
                                        '<span>'+(row.createMan?row.createMan:'')+
                                        '</span>&nbsp;&nbsp;&nbsp;'+
                                        '<span>'+(row.createDates?row.createDates:'')+
                                        '</span>'+
                                        '</div>'+
                                        '<div class="muchTxt">'+
